// TestKit.swift
// 极简 test runner：与 PetProfileKit 同款，自写避免依赖 XCTest / Swift Testing
// 在 Swift Package Manager 的 .executableTarget 里跑（无 app bundle 也能跑）
//
// 关键 API：
//   - Tests(suiteName:) → 注册容器
//   - register<TestFile>(tests) → 各测试 file 暴露注册函数
//   - tests.run() → 跑全部，[PASS]/[FAIL] 输出 + 退出码
//   - XCTAssert* / XCTUnwrap 抛 TestFailure
//
import Foundation
...
public final class Tests {
    ...
    public init(suiteName: String = "PetProfileRuntime") {
...
public func XCTAssertLessThan<T: Comparable>(_ a: @autoclosure () -> T, _ b: @autoclosure () -> T, _ message: String = "", file: StaticString = #file, line: UInt = #line) throws {
    if !(a() < b()) {
        throw TestFailure(name: "\(file):\(line)", message: "XCTAssertLessThan failed: \(a()) >= \(b()). \(message)")
    }
}
